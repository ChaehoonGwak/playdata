...
fac = [math.factorial(i) for i in range(1,6)] # 길이가 최대 5자리인 팩토리얼 진법 숫자가 주어지므로 5! 까지만 계산

while 1:
    num = sys.stdin.readline().rstrip()
    if num == '0': break # '0' 입력시 프로그램 종료
        
    answer = 0
    
    for idx, i in enumerate(str(num)[::-1]): # 자리수마다 해당되는 N!을 곱하여 answer에 더해준다
        answer += int(i)*fac[idx]
